 ...
 */
public class DBAMultiThreadCapable {
	...
	private static double[] DBAUpdate(double[] C, double[][] sequences,double[][]costMatrix,int[][]pathMatrix) {
		...
		for (double[] T : sequences) {
			...
			pathMatrix[0][0] = DBAMultiThreadCapable.NIL;

			for (i = 1; i < centerLength; i++) {
				costMatrix[i][0] = costMatrix[i - 1][0] + squaredDistance(C[i], T[0]);
				pathMatrix[i][0] = DBAMultiThreadCapable.UP;
			}
			for (j = 1; j < seqLength; j++) {
				costMatrix[0][j] = costMatrix[0][j - 1] + squaredDistance(T[j], C[0]);
				pathMatrix[0][j] = DBAMultiThreadCapable.LEFT;
			}

			for (i = 1; i < centerLength; i++) {
				for (j = 1; j < seqLength; j++) {
					indiceRes = DBAMultiThreadCapable.ArgMin3(costMatrix[i - 1][j - 1], costMatrix[i][j - 1], costMatrix[i - 1][j]);
			...
			while(pathMatrix[i][j]!=DBAMultiThreadCapable.NIL) {
				updatedMean[i]+=T[j];
				nElementsForMean[i]++;
